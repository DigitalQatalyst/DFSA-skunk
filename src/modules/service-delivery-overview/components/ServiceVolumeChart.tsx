...
import { LineChart, Line, AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, BarChart, Bar, Cell, LabelList } from 'recharts';
...
const ServiceVolumeChart = ({ timePeriod = 'week', serviceCategory = 'all', partnerType = 'all' }) => {
  ...
  const renderVolumeChart = () => (
    <ResponsiveContainer width="100%" height={400}>
      <AreaChart data={volumeData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
        ...
        >
          <LabelList dataKey="financialServices" position="top" style={{ fontSize: 12, fill: '#374151' }} />
        </Area>
        <Area
          type="monotone"
        ...
        >
          <LabelList dataKey="nonFinancialServices" position="top" style={{ fontSize: 12, fill: '#374151' }} />
        </Area>
      </AreaChart>
    </ResponsiveContainer>
  );
    ...
    <ResponsiveContainer width="100%" height={400}>
      <LineChart data={volumeData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
        ...
        <Line
          ...
          name="SLA Compliance"
        >
          <LabelList dataKey="slaCompliance" position="top" formatter={(v) => `${v}%`} style={{ fontSize: 12, fill: '#374151' }} />
        </Line>
        <Line
          type="monotone"
        ...
        >
          <LabelList dataKey="partnerEfficiency" position="top" formatter={(v) => `${v}%`} style={{ fontSize: 12, fill: '#374151' }} />
        </Line>
      </LineChart>
    </ResponsiveContainer>
  );
    ...
    <ResponsiveContainer width="100%" height={400}>
      <BarChart data={partnerComparisonData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
        ...
        <Bar dataKey="efficiency" name="Efficiency Score">
          ...
          <LabelList dataKey="efficiency" position="top" formatter={(v) => `${v}%`} style={{ fontSize: 12, fill: '#374151' }} />
        </Bar>
      </BarChart>
    </ResponsiveContainer>
  );
  ...
  const getChartTitle = () => {
    switch (chartView) {
      case 'volume':
        return 'Service Provider Delivery Volume';
      case 'efficiency':
