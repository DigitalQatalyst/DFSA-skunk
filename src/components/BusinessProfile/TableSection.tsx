...
export function TableSection({ title, columns, data }) {
  ...
  const handleSaveEditedRow = () => {
    setTableData(
      tableData.map((row) => (row.id === editingRow.id ? editingRow : row))
    );
    setEditingRow(null);
  };
  ...
  return (
    <div className="mt-4 border border-gray-200 rounded-lg overflow-hidden">
                ...
                {editingRow && editingRow.id === row.id ? (
                        ...
                        column.key !== "actions" && (
                          <td key={column.key} className="px-4 py-2">
                            <input
                              ...
                              className="border border-gray-300 rounded px-3 py-2 text-sm w-full min-h-[44px] min-w-0"
                            />
                          </td>
                        )
                    )}
                  ...
                  <>
                    {columns.map((column) => (
                      <td
                        key={column.key}
                        className="px-4 py-3 text-sm text-gray-700 max-w-0 truncate"
                      >
                        {column.key === "status" ? (
                          <span
      ...
      <div className="md:hidden">
        {paginatedData.map((row) => (
          <div key={row.id} className="p-4 border-b border-gray-200">
            {editingRow && editingRow.id === row.id ? (
              // Mobile editing mode
              ...
              <div>
                ...
                <div className="mt-3 flex justify-end space-x-2">
                  ...
                  <button
                    className="p-2 text-gray-500 hover:text-red-600 min-h-[44px] min-w-[44px]"
                    onClick={() => handleDeleteRow(row.id)}
                  >
                    <TrashIcon size={16} />
                  </button>
        ...
        <div className="bg-gray-50 px-4 py-3 flex items-center justify-between border-t border-gray-200">
          <div className="flex-1 flex justify-between sm:hidden">
            <button
              ...
              className={`relative inline-flex items-center px-4 py-2 text-sm font-medium rounded-md min-h-[44px] ${
                currentPage === 1
            ...
            <button
              ...
              className={`relative inline-flex items-center px-4 py-2 ml-3 text-sm font-medium rounded-md min-h-[44px] ${
            ...
            <div>
              ...
              >
                <button
                  onClick={() => setCurrentPage(Math.max(1, currentPage - 1))}
                  disabled={currentPage === 1}
                  className={`relative inline-flex items-center px-2 py-2 rounded-l-md border border-gray-300 bg-white text-sm font-medium min-h-[44px] ${
                    currentPage === 1
                      ? "text-gray-300"
                ...
                <button
                  ...
                  disabled={currentPage === totalPages}
                  className={`relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white text-sm font-medium min-h-[44px] ${
                    currentPage === totalPages
                      ? "text-gray-300"
                      : "text-gray-500 hover:bg-gray-50"
                  }`}
