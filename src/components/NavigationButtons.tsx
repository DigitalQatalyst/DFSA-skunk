        ...
        <div className="mt-10 pt-6 border-t border-gray-100 flex flex-col sm:flex-row justify-between items-center space-y-4 sm:space-y-0">
            ...
            <div className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3 w-full sm:w-auto">
                ...
                {!isLastStep ? (
                    ...
                    <Can I={isRevisit ? "update" : "create"} a="onboarding" passThrough>
                        {(allowed) => {
                            const button = (
                                <button
                                    type="button"
                                    ...
                                    className={`w-full sm:w-auto order-1 sm:order-2 px-5 py-3 ${isRevisit ? 'bg-blue-600 hover:bg-blue-700' : 'bg-green-600 hover:bg-green-700'
                                        } text-white rounded-md flex items-center justify-center transition-colors ${loading || !allowed ? 'opacity-70 cursor-not-allowed' : ''
                                        }`}
                                    aria-disabled={loading || !allowed}
                                >
                                    {loading ? (
                                        <>
                                    ...
                                    <TooltipProvider>
                                        <Tooltip>
                                            <TooltipTrigger asChild>
                                                <span className="inline-block w-full sm:w-auto">{button}</span>
                                            </TooltipTrigger>
                                            <TooltipContent>
                                                <p>You don't have permission to {isRevisit ? 'update' : 'create'} onboarding</p>
                                            </TooltipContent>
                                        </Tooltip>
                                    </TooltipProvider>
                                );
