...
const AvailableServices: React.FC<AvailableServicesProps> = ({ stageId }) => {
  ...
  useEffect(() => {
    const normalize = (value?: string | null | any) => {
              ...
              {filteredServices.slice(0, visibleCount).map((service) => (
                ...
                >
                  <a
                    ...
                    onClick={(e) => {
                      // Prevent navigation if it's an internal route that doesn't exist
                      if (service.url.startsWith('/stages/')) {
                        e.preventDefault()
                        e.stopPropagation()
                        console.warn('Prevented navigation to non-existent route:', service.url)
                      }
                    }}
                  >
...
const EnterpriseStages: React.FC = () => {
  ...
  // Prevent navigation to non-existent stage routes - redirect back if it happens
  useEffect(() => {
    if (activePopup) {
      const checkLocation = () => {
        // If somehow we navigated to a stage route, redirect back and keep modal open
        if (window.location.pathname.startsWith('/stages/')) {
          console.warn('Prevented navigation to non-existent stage route, staying on home')
          window.history.replaceState(null, '', '/')
          // Force re-render to show modal again if it was lost
  ...
  const handleOpenPopup = (stageId: string) => {
    // Prevent any navigation when opening modal
    setActivePopup(stageId)
    // Ensure we stay on the current page
    if (window.location.pathname !== '/') {
      // If somehow we're not on home, prevent navigation
      console.log('Modal opened, preventing navigation')
    }
  }
  // Handle closing the popup - ensure no navigation happens
  const handleClosePopup = () => {
    setActivePopup(null)
    // Explicitly prevent any navigation when closing
  }
  return (
          ...
          {stages.map((stage, index) => (
            ...
            >
              <StageCard
                ...
                onClick={() => {}} // Not used - card click opens modal instead
                onShowDetails={() => handleOpenPopup(stage.id)} // Card click and button open popup
                index={index}
