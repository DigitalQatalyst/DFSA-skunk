...
export function CommunityGrowthChart({
  ...
  const fetchGrowthData = async () => {
    ...
    const query = supabase.from('memberships').select('joined_at').in('community_id', communityIds).gte('joined_at', startDate.toISOString()).order('joined_at', {
      ascending: true
    });
    const [memberships] = await safeFetch(query);
    if (memberships) {
      // Group by date
      const dates = viewMode === 'weekly' ? eachWeekOfInterval({
        start: startDate,
        end: new Date()
      }).map(d => startOfWeek(d)) : eachDayOfInterval({
        start: startDate,
        end: new Date()
      });
      ...
      <div className="p-6 border-b border-gray-200">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Button variant={viewMode === 'weekly' ? 'default' : 'secondary'} size="sm" onClick={() => setViewMode('weekly')} className="text-xs px-3 py-1">
              Weekly
            </Button>
            <Button variant={viewMode === 'monthly' ? 'default' : 'secondary'} size="sm" onClick={() => setViewMode('monthly')} className="text-xs px-3 py-1">
              Monthly
            </Button>
          </div>
        </div>
      ...
      <div className="p-6">
        <ResponsiveContainer width="100%" height={300}>
          <LineChart data={data}>
            ...
            <Line type="monotone" dataKey="members" stroke="#2563eb" strokeWidth={2} dot={{
            fill: '#2563eb',
            r: 4
          }} activeDot={{
            r: 6
