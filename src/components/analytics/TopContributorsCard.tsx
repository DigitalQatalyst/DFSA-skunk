...
export function TopContributorsCard({
  ...
  const fetchTopContributors = async () => {
    ...
    try {
      // Fetch posts
      const postsQuery = supabase.from('posts').select('created_by').in('community_id', communityIds).gte('created_at', thirtyDaysAgo.toISOString()).eq('status', 'active');

      // Fetch comments
      const commentsQuery = supabase.from('comments').select('created_by, post_id').gte('created_at', thirtyDaysAgo.toISOString()).eq('status', 'active');
      const [postsData, commentsData] = await Promise.all([safeFetch(postsQuery), safeFetch(commentsQuery)]);

      // Get post IDs in our communities
      const postsInCommunitiesQuery = supabase.from('posts').select('id').in('community_id', communityIds);
      ...
      if (commentsData[0]) {
        commentsData[0].filter((comment: any) => postIds.has(comment.post_id)).forEach((comment: any) => {
      ...
      const topUserIds = Array.from(contributionMap.entries()).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([userId]) => userId);
      if (topUserIds.length > 0) {
        // Fetch user details
        const usersQuery = supabase.from('users_local').select('id, username, avatar_url').in('id', topUserIds);
  ...
  return <div className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden animate-fade-in">
      ...
      <div className="p-6">
        {contributors.length === 0 ? <div className="border border-dashed border-gray-300 rounded-lg p-6 text-center">
            <p className="text-sm text-gray-500">No contributors yet</p>
          </div> : <div className="space-y-4">
            {contributors.map((contributor, index) => <Link key={contributor.user_id} to="/profile" className="flex items-center gap-3 p-3 border border-gray-100 rounded-lg hover:border-gray-200 hover:shadow-sm transition-all">
                <div className="relative">
                  ...
                  {index === 0 && <div className="absolute -top-1 -right-1 bg-amber-100 rounded-full p-1">
