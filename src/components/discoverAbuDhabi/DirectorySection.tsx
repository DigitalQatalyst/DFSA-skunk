...
const DirectorySection = () => {
  ...
  // Load initial profiles from API
  useEffect(() => {
    const load = async () => {
      try {
        const { items } = await fetchDirectoryItems({ page: 1, limit: 9 });
        const mapped = items.map((d: any, idx: number) => ({
          ...
          phone: d.contactPhone ?? d.contact_phone ?? '',
          email: d.contactEmail ?? d.contact_email ?? '',
          website: d.website ?? '',
          address: d.address ?? d.location ?? '',
          founded: String(d.establishedYear ?? d.established_year ?? ''),
  ...
  const handleLoadMore = async () => {
    setIsLoading(true);
    try {
      const { items } = await fetchDirectoryItems({ page: 1, limit: 30 });
      const mapped = items.map((d: any, idx: number) => ({
        id: d.id ?? idx + 1,
        name: d.name,
        logo: d.logo ?? '',
      ...
      const newProfiles = [...profiles, ...mapped];
