...
export const MediaCard: React.FC<MediaCardProps> = ({
  ...
  // Updated navigation handler - prefer explicit CTA href when provided
  const handleNavigation = (e?: React.MouseEvent) => {
    if (e) {
      e.stopPropagation();
    }
    const targetHref = (cta as any)?.href || detailsHref
    if (!targetHref) {
  ...
  // Convert badges into up to three pills: Type, Category, Business Stage (in that order)
  // Variants: Type -> primary (blue), Category -> info (purple), Stage -> success (green)
  const pillItems = badges.slice(0, 3).map((badge, index) => ({
    text: badge,
    variant: (index === 0
      ? "primary"
      : index === 1
      ...
      : "success") as const,
    ariaPrefix:
      index === 0 ? "Type" : index === 1 ? "Category" : "Business Stage",
  }));
  const hasPills = pillItems.length > 0;
          ...
          {type === "video" && (
            <div className="absolute inset-0 flex items-center justify-center">
              <button
                ...
                className={`flex items-center justify-center w-16 h-16 rounded-full bg-white bg-opacity-90 hover:bg-opacity-100 transition-all duration-300 transform ${
          ...
          {type === "podcast" && (
            <div className="absolute inset-0 flex items-center justify-center">
              <button
                ...
                className={`flex items-center justify-center w-16 h-16 rounded-full bg-white bg-opacity-90 hover:bg-opacity-100 transition-all duration-300 transform ${
                  isHovered ? "scale-110" : "scale-100"
                } ${isPlaying && audioUrl ? "bg-green-500 bg-opacity-90" : ""}`}
    ...
    >
      ...
      >
        ...
        <div className="mb-4 flex-shrink-0">
          {typeLabel && (
            <span
              className="inline-block mb-2 px-2 py-0.5 rounded-full text-[11px] font-semibold uppercase tracking-wide bg-blue-50 text-blue-700"
        ...
        {/* Pills Row: Category then Business Stage */}
        {hasPills && (
          <div className="mb-3 flex-shrink-0 static">
            <div className="flex flex-wrap gap-2 static">
              {pillItems.map((pill, index) => (
                <span
                  key={index}
                  className={`inline-flex items-center ${
                    designTokens.spacing.pill.padding
                  } rounded-full text-xs font-medium whitespace-nowrap static z-auto ${
                    tagVariants[pill.variant]
                  }`}
                  tabIndex={0}
                  aria-label={`${pill.ariaPrefix}: ${pill.text}`}
                  title={`${pill.ariaPrefix}: ${pill.text}`}
                >
                  {pill.text}
                </span>
              ))}
        ...
        {/* Metadata Rows Under Pills: Date row then Provider row */}
        <div className="mb-4 flex-shrink-0">
          {/* Row 1: Date */}
          {(metadata?.date || metadata?.updated) && (
            <div
              className={`${designTokens.typography.metadata.size} ${designTokens.typography.metadata.color} flex items-center gap-2`}
            >
              <Calendar size={12} aria-label="Published date icon" />
              <span
                aria-label={`${metadata?.updated ? "Updated" : "Published"}: ${
          ...
          {/* Row 2: Provider/Author chain */}
          {(metadata?.primaryAuthor || metadata?.author || metadata?.speaker) && (
            <div className={`${designTokens.typography.metadata.size} ${designTokens.typography.metadata.color} ${designTokens.typography.metadata.weight} ${designTokens.typography.metadata.lineHeight} flex items-center gap-2 min-w-0 max-w-full w-full`}>
              {metadata?.primaryAuthor ? (
                <>
                  <Building size={12} className="flex-shrink-0" aria-label="Author icon" />
                  <div className="min-w-0 flex flex-col">
                    <span
                      className="truncate min-w-0"
                      title={`By: ${metadata.primaryAuthor.name}${metadata.primaryAuthorOrganization ? ` • ${metadata.primaryAuthorOrganization}` : ''}`}
                      aria-label={`By: ${metadata.primaryAuthor.name}${metadata.primaryAuthorOrganization ? ` • ${metadata.primaryAuthorOrganization}` : ''}`}
                    >
                      {metadata.primaryAuthor.name}
                      {typeof metadata.additionalAuthorsCount === 'number' &&
                        metadata.additionalAuthorsCount > 0 && (
                          <span className="text-xs text-gray-500 ml-1" title={(metadata.additionalAuthorsNames || []).join(', ')}>
                            +{metadata.additionalAuthorsCount} more
                          </span>
                        )}
                    </span>
                    {metadata.primaryAuthorOrganization && (
                ...
                <>
                  <Building size={12} className="flex-shrink-0" aria-label="Provider icon" />
                  <span
                    className="truncate min-w-0"
                    title={`By: ${metadata?.author || metadata?.speaker}`}
                    aria-label={`By: ${metadata?.author || metadata?.speaker}`}
                  >
          ...
          {/* Event-specific metadata (kept compact and secondary) */}
          {type === "event" && (
          ...
          {/* Report/Toolkit supplemental metadata panel */}
          {(type === "report" || type === "toolkit") && (
            <div className="mt-2 flex justify-between items-center text-xs text-gray-500 p-2 bg-gray-50 rounded">
          ...
          {type === "event" && secondaryCta ? (
            <div className="flex gap-2">
              <button
                ...
                className={`flex-1 px-4 py-3 text-sm font-bold text-white rounded-md transition-colors flex items-center justify-center gap-2 ${
                  isCtaDisabled
            ...
            <button
              ...
              disabled={isCtaDisabled}
              aria-disabled={isCtaDisabled}
              className={`w-full px-4 py-3 text-sm font-bold text-white rounded-md transition-colors flex items-center justify-center gap-2 ${
                isCtaDisabled ? "bg-gray-400 cursor-not-allowed" : ctaColorClass
              }`}
