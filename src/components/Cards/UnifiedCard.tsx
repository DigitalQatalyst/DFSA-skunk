...
export const UnifiedCard: React.FC<UnifiedCardProps> = ({
  ...
  // Render pill (inline, normal flow — no absolute positioning)
  const renderPill = () => {
    ...
    return (
      <span
        className={`${designTokens.spacing.pill.padding} rounded-full text-[11px] font-semibold uppercase tracking-wide inline-flex items-center whitespace-nowrap ${pillVariants[content.pill.variant || 'warning']}`}
        aria-label={`Label: ${content.pill.text}`}
        title={content.pill.text}
      >
        {content.pill.icon && <span className="mr-1" aria-hidden="true">{content.pill.icon}</span>}
        {content.pill.text}
      </span>
    );
  };
      ...
      <div className={`${designTokens.spacing.card.padding} flex flex-col h-full relative`}>
        ...
        <div className="flex items-start mb-5">
          ...
          <div className="flex-grow min-h-[72px] flex flex-col justify-center">
            {/* Inline Pill above title */}
            {renderPill() && <div className="mb-2">{renderPill()}</div>}
