...
import {
  ...
  getSupabase,
} from "../../lib/supabase/defaultClient";
import {
  listPublicMedia,
  mapGridToCard,
...
export const MarketplacePage: React.FC<MarketplacePageProps> = ({
  ...
  // Knowledge Hub: initial load (keyset pagination), with simple memory cache
  const loadKHInitial = useCallback(async () => {
    ...
    try {
      ...
      const cacheKey = `q:${debouncedSearch}|limit:${pageLimit}`;
  ...
  }, [marketplaceType, debouncedSearch, khPageSize, computedPageSize]);

  // Knowledge Hub: load more
  const loadKHMore = useCallback(async () => {
    if (marketplaceType !== "knowledge-hub" || khFetching || !khHasMore) return;
    setKhFetching(true);
    try {
      const pageLimit = Math.max(1, khPageSize || computedPageSize);
      const { items: gridItems, nextCursor } = await listPublicMedia({
      ...
      setFilteredItems((prev) => [...prev, ...mapped]);
      setKhCursor(nextCursor);
      setKhHasMore(Boolean(nextCursor));
    } catch (e) {
      console.warn("Knowledge Hub load more failed", e);
    } finally {
      setKhFetching(false);
    }
  }, [
  ...
  // Reset page when filters/search/page size change (Knowledge Hub only)
  useEffect(() => {
  ...
  // Navigate KH pages with keyset backing (Prev/Next)
  ...
  useEffect(() => {
    const loadItems = async () => {
      ...
      try {
        // Handle Knowledge Hub (lean view + keyset)
        if (marketplaceType === "knowledge-hub") {
  ...
  const handleKnowledgeHubFilterChange = useCallback(
    (filter: string) => {
      setActiveFilters((prev) => {
        ...
        const mediaTypeFilter = filterConfig.find((c) => c.id === "mediaType");
        const isMediaTypeFilter = mediaTypeFilter?.options.some(
          (opt) => opt.name === filter
        );
        ...
        if (isMediaTypeFilter) {
          ...
          if (newMediaTypes.length > 0) {
            ...
            return newFilters.filter((f) => {
              const isFormatFilter = formatFilter?.options.some(
                (opt) => opt.name === f
              );
              if (isFormatFilter) {
  ...
  useEffect(() => {
    ...
    const matchesActiveFilters = (it: any): boolean => {
      ...
      return Object.keys(filtersByCategory).every((catId) => {
        ...
        if (!itemVal) {
          // fallback to tags if missing
          return (
            Array.isArray(it.tags) &&
            vals.some((f) =>
              it.tags.some((t: string) => normalize(t) === normalize(f))
            )
          );
        }
        ...
        <div className="flex flex-col xl:flex-row gap-6">
          ...
          <div className="xl:w-3/4">
            {loading ? (
              ...
              (!skipGraph && (facetError || productError)) ||
              courseError ? (
              <ErrorDisplay
                message={
                  error ||
