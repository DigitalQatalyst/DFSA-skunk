...
export interface ProviderTabProps {
  ...
  marketplaceType: "courses" | "financial" | "events" | "non-financial";
  item: any;
}
...
const MOCK_DATA_POOL = {
  ...
  events: {
    names: [
      "Abu Dhabi Events Hub",
      "Emirates Conference Center",
      "Gulf Business Events",
      "UAE Networking Platform",
      "Professional Events Group",
    ],
    taglines: [
      "Connecting businesses through events",
      "Premier event organizer in the UAE",
      "Creating meaningful business connections",
      "Your gateway to networking success",
      "Excellence in event management",
    ],
    descriptions: [
      "Leading event organizer specializing in business conferences, workshops, and networking events across the UAE.",
      "Premier provider of professional events and conferences, connecting entrepreneurs and business leaders.",
      "Innovative event platform delivering world-class business networking and learning opportunities.",
      "Established event management company specializing in corporate events and professional development.",
      "Award-winning event organizer focused on creating impactful business connections and learning experiences.",
    ],
    expertise: [
      ["Business Conferences", "Networking Events", "Workshops", "Trade Shows"],
      ["Corporate Events", "Professional Development", "Industry Forums", "Executive Summits"],
      ["Startup Events", "Pitch Competitions", "Innovation Workshops", "Business Expos"],
      ["Training Events", "Seminars", "Panel Discussions", "Masterclasses"],
      ["Virtual Events", "Hybrid Conferences", "Webinars", "Online Networking"],
    ],
    locations: ["Abu Dhabi, UAE", "Dubai, UAE", "Multiple Venues, UAE", "Virtual Platform", "ADNEC, Abu Dhabi"],
    established: ["2010", "2012", "2015", "2017", "2019"],
    contacts: [
      "info@businessevents.ae",
      "events@networking.ae",
      "register@conferences.ae",
      "contact@eventshub.ae",
      "support@profevents.ae",
    ],
    websites: [
    ...
    services: ["100+ Annual Events", "50+ Networking Sessions", "75+ Workshops & Seminars", "25+ Major Conferences", "60+ Training Events"],
  },
  "non-financial": {
    names: [
...
const getProviderData = (providerName: string, marketplaceType: "courses" | "financial" | "events" | "non-financial") => {
...
const ProviderTab: React.FC<ProviderTabProps> = ({
  ...
  const displayDescription = (provider.description && provider.description.trim() !== "")
    ? provider.description
    : (provider.shortDescription && provider.shortDescription.trim() !== "")
      ? provider.shortDescription
      : mockData.description;
  const displayWebsite = (provider.website && provider.website.trim() !== "") ? provider.website : mockData.website;
  const displayLocation = (provider.location && provider.location.trim() !== "") ? provider.location : mockData.location;
  const displayContact = (provider.emailAddress && provider.emailAddress.trim() !== "") ? provider.emailAddress : mockData.contact;
  const displayServices = typeof provider.publicServicesCount === 'number' && provider.publicServicesCount > 0
    ? `${Math.floor(provider.publicServicesCount/10)*10}+ Public Services`
    : mockData.services; 
  return (    
