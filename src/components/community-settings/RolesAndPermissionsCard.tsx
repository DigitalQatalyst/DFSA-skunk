...
export function RolesAndPermissionsCard({
  ...
  const fetchMembers = async () => {
    ...
    try {
      ...
      const communityQuery = supabase.from('communities').select('created_by').eq('id', communityId).maybeSingle();
      const [communityData] = await safeFetch(communityQuery);
      const ownerId = communityData?.created_by;

      // Fetch members
      const membersQuery = supabase.from('memberships').select(`
          id,
          user_id,
          joined_at,
  ...
  const handleRemoveMember = async () => {
    ...
    setRemoving(true);
    const query = supabase.from('memberships').delete().eq('id', memberToRemove.id);
  ...
  const getRoleBadge = (member: Member) => {
    if (member.is_owner) {
      return <Badge className="bg-blue-100 text-blue-800 border-blue-200 hover:bg-blue-100">
      ...
      <div className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden animate-fade-in">
        ...
        <div className="p-6">
          <div className="space-y-3">
            {members.map(member => <div key={member.id} className="border border-gray-100 rounded-lg p-4 flex items-center justify-between gap-4 hover:border-gray-200 transition-colors">
                ...
                <div className="flex items-center gap-2">
                  {getRoleBadge(member)}
                  {isOwner && !member.is_owner && member.user_id !== currentUserId && <Button variant="secondary" size="sm" onClick={() => setMemberToRemove(member)} className="px-3 py-1 text-xs text-red-600 hover:bg-red-50 border-red-200">
      ...
      <AlertDialog open={!!memberToRemove} onOpenChange={open => !open && setMemberToRemove(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleRemoveMember} disabled={removing} className="bg-red-600 hover:bg-red-700 text-white">
