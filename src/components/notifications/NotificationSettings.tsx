...
export function NotificationSettings() {
  ...
  const fetchSettings = async () => {
    if (!user) return;
    const query = supabase.from('users_local').select('notification_settings').eq('id', user.id).maybeSingle();
  ...
  const handleToggle = async (key: keyof NotificationSettingsData, value: boolean) => {
    ...
    const query = supabase.from('users_local').update({
  ...
  return <div className="space-y-4">
      ...
      return <div key={item.key} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
            ...
            <Switch id={item.key} checked={settings[item.key]} onCheckedChange={checked => handleToggle(item.key, checked)} />
          </div>;
    })}
    </div>;
}