      ...
      <SheetContent className="w-full sm:max-w-lg overflow-y-auto bg-white">
        ...
        <div className="space-y-6 mt-6">
          {loading ? <div className="space-y-4">
            ...
            </div> : notifications.length === 0 ? <div className="border border-dashed border-gray-300 rounded-lg p-6 text-center">
              ...
              {groupedNotifications.today.length > 0 && <div>
                  ...
                  <div className="space-y-2">
                    {groupedNotifications.today.map(notification => <NotificationCard key={notification.id} notification={notification} onMarkAsRead={handleMarkAsRead} onDelete={handleDelete} onClose={() => onOpenChange(false)} />)}
                  </div>
                </div>}

              {groupedNotifications.thisWeek.length > 0 && <div>
                  <h3 className="text-xs font-semibold text-gray-500 uppercase mb-3">
                    This Week
                  </h3>
                  <div className="space-y-2">
                    {groupedNotifications.thisWeek.map(notification => <NotificationCard key={notification.id} notification={notification} onMarkAsRead={handleMarkAsRead} onDelete={handleDelete} onClose={() => onOpenChange(false)} />)}
                  </div>
                </div>}
              ...
              {groupedNotifications.earlier.length > 0 && <div>
                  ...
                  <div className="space-y-2">
                    {groupedNotifications.earlier.map(notification => <NotificationCard key={notification.id} notification={notification} onMarkAsRead={handleMarkAsRead} onDelete={handleDelete} onClose={() => onOpenChange(false)} />)}
                  </div>
                </div>}
            </>}
        </div>
