...
import {
  NewsCard as CardsNewsCard,
  ResourceCard as CardsResourceCard,
  EventCard as CardsEventCard,
...
export const NewsCard = ({
  ...
  onQuickView,
  onReadMore = () => {},
  // Add default empty function to make it optional in the wrapper
  ...props
}) => {
  const newsItem = {
    id: content.title,
...
export const ResourceCard = ({
  ...
  onQuickView,
  onAccessResource = () => {},
  // Add default for required callback
  onDownload,
  ...props
}) => {
  ...
  const resourceItem = {
    id: content.title,
    title: content.title,
    category: content.category || content.type || 'Resources',
    description: content.description,
    tags: content.tags || [],
    resourceType: mapResourceType(content.type),
    fileSize: content.fileSize || (Math.random() * 5).toFixed(1) + ' MB',
    downloadCount:
      content.downloadCount || Math.floor(Math.random() * 1000) + 100,
