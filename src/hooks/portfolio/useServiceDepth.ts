...
export const useServiceDepth = (startDate?: string, endDate?: string, serviceCategory?: string, providerId?: string) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      if (!azureId) return;
      try {
        let url = `${API_BASE_URL}/api/portfolio/service-depth/?azureId=${azureId}`;
        ...
        console.error('Failed to fetch service depth:', error);
      } finally {
        setLoading(false);
      }
    };
