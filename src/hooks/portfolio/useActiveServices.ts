...
export const useActiveServices = (startDate?: string, endDate?: string, serviceCategory?: string, providerId?: string) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      if (!azureId) return;
      try {
        let url = `${API_BASE_URL}/api/portfolio/active-services?azureId=${azureId}`;
