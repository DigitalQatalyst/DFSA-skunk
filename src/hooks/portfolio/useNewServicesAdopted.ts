...
export const useNewServicesAdopted = (startDate?: string, endDate?: string, serviceCategory?: string, providerId?: string) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      if (!azureId) return;
      try {
        let url = `${API_BASE_URL}/api/portfolio/active-services/new-services-adopted?azureId=${azureId}`;
        ...
        console.error('Failed to fetch new services adopted:', error);
      } finally {
        setLoading(false);
      }
    };
