...
export const useProviderConcentration = (groupBy?: string, startDate?: string, endDate?: string, serviceCategory?: string, providerId?: string) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await fetch(`${API_BASE_URL}/api/portfolio/provider-concentration?${params.toString()}`);
        const result = await response.json();
        setData(result.data);
      } catch (error) {
        console.error('Failed to fetch provider concentration:', error);
      } finally {
        setLoading(false);
      }
    };
