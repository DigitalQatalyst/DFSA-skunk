...
// Known media types (both singular and plural labels) – used to recognize
// a "media type" filter regardless of how the UI labels it. We map these
// labels to the canonical DB type values before querying Supabase.
// FIX: Include singular labels (e.g., 'Video') to avoid client-side filtering
// only on the first page when the UI emits singular type names.
const KNOWN_TYPES = [
  'News',
  'Report', 'Reports',
  'Tool', 'Toolkits & Templates', 'Toolkit',
  'Guide', 'Guides',
  'Event', 'Events',
  'Video', 'Videos',
  'Podcast', 'Podcasts',
      ...
      try {
        ...
        try {
          ...
          if (filters && filters.length > 0) {
            // Separate media-type filters from other filters. Accept both
            // singular and plural labels, and map them to DB type values.
            const typeFiltersRaw = (filters || []).filter((f) => KNOWN_TYPES.includes(String(f))) as string[]
            // Canonical mapping to Supabase v_media_public.type values
            const typeMap: Record<string,string> = {
              ...
              // Toolkits/Templates (stored as Tool)
              'Tool': 'Tool', 'Toolkit': 'Tool', 'Toolkits & Templates': 'Tool',
              // Images (if present)
              'Image': 'Image', 'Images': 'Image',
            }
            const typeFilters = typeFiltersRaw.map((t) => typeMap[t] || t)
          ...
          const mapped = (data || []).map((row: any) => {
            ...
            // Include category/domain and business stage so pills can render
            const category = (row as any).category || (row as any).domain || null
            const domain = (row as any).domain || null
            const businessStage = (row as any).business_stage || (row as any).businessStage || null
            // Prefer summary; fallback to body_html/article_body_html and strip
            ...
            const rawDesc = (row as any).summary || (row as any).body_html || (row as any).article_body_html || ''
            const safeDescription = stripHtml(String(rawDesc || ''))
            ...
            return mapApiItemToCardProps({
              ...
              downloadUrl: (row as any).report_document_url || (row as any).tool_document_url || null,
