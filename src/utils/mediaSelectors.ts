...
export const getPosterUrl = (item: any): string => {
  // Type-aware fallback posters
  const TYPE_FALLBACKS: Record<string, string> = {
    // Provided defaults
    podcast:
      'https://images.unsplash.com/photo-1519874179391-3ebc752241dd?ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&q=80&w=2070',
    video:
      'https://plus.unsplash.com/premium_photo-1710409625145-f33714bdf870?ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&q=80&w=2070',
    event:
    ...
    report:
      'https://images.unsplash.com/photo-1562654501-a0ccc0fc3fb1?ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&q=80&w=1332',
    tool:
      'https://images.unsplash.com/photo-1562654501-a0ccc0fc3fb1?ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&q=80&w=1332',
    'toolkits & templates':
      'https://images.unsplash.com/photo-1562654501-a0ccc0fc3fb1?ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&q=80&w=1332',
    // Keep existing/generic for others
    guide:
  ...
  const inferType = (): string => {
    const t = (item?.mediaType || item?.type || '').toString().toLowerCase()
    if (t) return t
    const fmt = (item?.format || '').toString().toLowerCase()
    if (fmt.includes('video')) return 'video'
    if (fmt.includes('podcast') || fmt.includes('audio')) return 'podcast'
    if (fmt.includes('event')) return 'event'
    if (fmt.includes('report')) return 'report'
    if (fmt.includes('guide')) return 'guide'
    if (fmt.includes('tool')) return 'tool'
    const tags = Array.isArray(item?.tags)
      ? item.tags.map((x: any) => String(x).toLowerCase()).join(' ')
      : ''
  ...
  const fallbackPosterUrl = TYPE_FALLBACKS[inferType()] || TYPE_FALLBACKS['resource']
